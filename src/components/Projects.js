...
import { Container, Grid, Typography, Dialog, DialogContent, IconButton, useTheme, useMediaQuery, Card, CardMedia, Box, CardContent, Skeleton } from '@mui/material';
...
const projects = [
  {
    ...
    images: [Sharewell, Sharewell_dash, Sharewell_tool],
  ...
  },
  {
    ...
    images: [Data_Dash, Data_client, Data_Event],
    ...
    fullDesc: [
      { title: "🦸 The Mission", description: "Teamed up to help Community Health Workers in Houston do their thing better. Food aid, education, client tracking — all in one app that doesn't suck." },
      { title: "✨ Features We Added", description: "Event management, service tracking, client lists — basically turned chaos into organized chaos. CHWs can now find info without digging through spreadsheets." },
      { title: "🔐 Security (Because Duh)", description: "Added roles and permissions so not everyone can see everything. Intern can't delete the database. You're welcome." },
      { title: "🏢 Multi-Org Support", description: "One database, multiple non-profits, zero drama. Each org gets their own sandbox to play in." },
      { title: "📸 Profile Pics & Polish", description: "File uploads for profile pictures because clients deserve to be more than just a row in a table. Also optimized the heck out of everything." },
    ],
  },
  {
    title: 'Discord Bot: Pulse',
    images: [Pulse, role, embed, info_server],
    ...
    fullDesc: [
      { title: "👑 Server Admin Stuff", description: "Kick, ban, manage roles — all the power trips you need for keeping your server from becoming a lawless wasteland. Mods who've witnessed Discord drama will weep tears of joy." },
      { title: "🎨 Be Your Own Rainbow", description: "Let users pick their name colors because we're all just trying to stand out in a sea of gray usernames. Admins can assign role colors too for maximum ✨aesthetic✨ energy." },
      { title: "📝 Everything Gets Logged", description: "Who deleted what? Who rage-quit at 3am? Pulse remembers everything like that one friend who never forgets that embarrassing thing you said 5 years ago." },
      { title: "🎭 Embeds & Role Reactions", description: "Gorgeous custom embeds and role reactions that'll make your server look so professional, people will assume there's an actual adult running things." },
      { title: "🔧 Dev Transparency", description: "Recent commits display so users can watch me fix bugs in real-time. It's like a reality show but sadder and with more semicolons." },
      { title: "🚀 Coming Soon™", description: "Free game alerts from Steam, Epic, GOG. Because your backlog isn't big enough and Pulse believes in enabling your poor life choices." },
    ],
  },
];
  ...
  return (
    <Box sx={{ position: 'relative', height: 200 }}>
      ...
      <CardMedia
        ...
        sx={{ height: 200, display: loaded ? 'block' : 'none' }}
      />
    </Box>
  );
}
        ...
        <Grid container spacing={3} justifyContent="center">
          {projects.map((project, index) => (
            <Grid item xs={12} sm={6} md={4} key={index}>
              <motion.div variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0, transition: { duration: 0.4, ease: 'easeOut' } } }}>
                <Card
          ...
          <DialogContent sx={{ overflowY: 'auto', maxHeight: '80vh', position: 'relative' }}>
            <div style={{ position: 'relative', height: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#000' }}>
              ...
              <CardMedia
                ...
                sx={{ maxHeight: '400px', width: '100%', objectFit: 'contain' }}
              />
              <IconButton
            ...
            <CardContent sx={{ paddingTop: theme.spacing(2) }}>
              <Typography gutterBottom variant="h5" component="div">{selectedProject.title}</Typography>
